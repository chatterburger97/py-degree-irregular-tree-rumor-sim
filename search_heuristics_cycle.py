...
class Graph(object):
    ...
    def construct_underlying_graph(self):
        ...
        for i in range(1, N + 1):
            self.G[i].max_degree = randint(1, d)
    ...
    def spread_rumor(self, n):
        ...
        self.child_sum(root, Gn)  # compute t^root_v for each node v
    ...
    def kruskal(self):
        ...
        while(self.edge_heap):
            length, (v1, v2) = self.heappop(self.edge_heap)
            if p.find(v1) is not p.find(v2):
                p.union(v1, v2)
                p.add_edge(length, v1, v2)
            else:
    ...
    def rumor_center2(self, k):
        for i in self.Gn[k].children:
            self.Gn[i].rc = self.Gn[k].rc * \
                (self.Gn[i].descendant_num /
                 (10 - float(self.Gn[i].descendant_num)))
    ...
    def dfs(self, k, path=[], possibility=1, degsum=0):
        ...
        for current in self.Gn[k].neighbor:
            if current not in path:
                path, possibility = self.dfs(
                    current, path, possibility, degsum)
        return path, possibility
    ...
    def max_deg_search(self, k, path=[], neighbor_heap=[], possibility=1):
        ...
        for i in self.Gn[k].neighbor:
            if i not in path:
                heappush(neighbor_heap,
                         (-self.G[i].degree, -self.Gn[i].descendant_num, i))
        if neighbor_heap:
            possibility = possibility / float(len(neighbor_heap))
            path, possibility = self.max_deg_search(
                (heappop(neighbor_heap))[2], path, neighbor_heap, possibility)
        return path, possibility
