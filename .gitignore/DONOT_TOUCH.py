...
def max_deg_search(source,path=[],neighbor_heap=[],possibility=1):
        ...
        for i in Gn[source].neighbor:
                if i not in path:
                        heappush(neighbor_heap,(G[i].degree,i))
        if neighbor_heap:
...
for val in infected_group :
	path, possibility = bfs(val, 2)
	print ('ascending bfs\n', path, possibility)
	if float(possibility) > max_ascending_bfs :
		max_ascending_bfs = float(possibility)
		max_node_ascending_bfs = val

print 'max_ascending_bfs is ', float(max_ascending_bfs), 'for the max node', max_node_ascending_bfs

dist_map = dict()
# populate the dist_map with the distances of each node from the actual start node

dijkstra.dijkstra(G, infected_group, dist_map, source)

print '\nthe hop error between the actual start node and all the other nodes in the infected graph'
# access the hop error between the actual start node and the calculated start_node like so
for infected in infected_group :
	print infected,dist_map[infected],
	if infected == max_node_ascending_bfs :
		print ' <<<<<<<'
	else :
		print ''

print "*********************************************************************"
max_descending_bfs = 0
print ('descending bfs\n',bfs(root,2))
for val in infected_group :
	path, possibility = bfs(val, 2)
	print ('descending bfs\n', path, possibility)
	if float(possibility) > max_descending_bfs :
		max_descending_bfs = float(possibility)
		max_node_descending_bfs = val

print 'max_descending_bfs is ', float(max_descending_bfs), 'for the max node', max_node_descending_bfs
...
for infected in infected_group :
	print infected,dist_map[infected],
	if infected == max_node_descending_bfs :
		print ' <<<<<<<'
	else :
		print ''