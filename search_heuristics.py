...
class Graph(object):
    ...
    def __init__(self, N, max_degree,n):
        self.G = []
        self.N = N
        self.n = n
    ...
    def pick_source(self):
        ...
        while len(self.G[real_source].neighbor)==1:
            real_source=(real_source*randint(1,self.N))%self.N;
    ...
    def rumor_center1(self, k):
        ...
        if len(self.Gn[k].children) > 0:
            for i in self.Gn[k].children:
                ...
                if k == self.root:
                    self.Gn[k].rc = factorial(self.n) / (self.Gn[k].descendant_num * temp_mul)
    ...
    def rumor_center2(self, k):
        for i in self.Gn[k].children:
            self.Gn[i].rc = self.Gn[k].rc * (self.Gn[i].descendant_num / (self.n - float(self.Gn[i].descendant_num)))
