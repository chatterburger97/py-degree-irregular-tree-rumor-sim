...
class Graph(object) :
	...
	def spread_rumor(self, n):
		...
		root_index=randint(0,len(self.infected_group)-1) #root_index is from 0~n-1
	...
	def child_sum(self, k):
		...
		if len(self.Gn[k].children)>0:
			for i in self.Gn[k].children:
				...
				self.Gn[k].descedant_num = 1 + temp_sum
			return 1 + temp_sum   
		else:
			self.Gn[k].descedant_num = 1
			return 1
	...
	def rumor_center1(self, k):
		...
		if len(self.Gn[k].children)>0:
			for i in self.Gn[k].children:
				...
				if k==root :
					self.Gn[k].rc=factorial(n)/(self.Gn[k].descedant_num*temp_mul)
			return self.Gn[k].descedant_num*temp_mul
		else:
			return 1  

	def rumor_center2(self, k):
		for i in self.Gn[k].children:
			self.Gn[i].rc=self.Gn[k].rc*(self.Gn[i].descedant_num/(n-float(self.Gn[i].descedant_num)))
			self.rumor_center2(i)

	#function to sort neighbors according to degree
	def deg_sort(self, i, Reverse):
	        return sorted(self.Gn[i].neighbor,key=lambda v:(self.Gn[v].degree,self.Gn[v].descedant_num),reverse=Reverse)
