...
def child_sum(k):
        ...
        if len(Gr[k].children)>0:
                ...
                Gr[k].descedant_num=1+temp_sum
                return 1+temp_sum   
        else:
                Gr[k].descedant_num=1
                return 1
...
def rumor_center1(k):
        ...
        if len(Gr[k].children)>0:
                for i in Gr[k].children:
                        ...
                        if k==root :
                            Gr[k].rc=factorial(n)/(Gr[k].descedant_num*temp_mul)
                return Gr[k].descedant_num*temp_mul
        else:
                return 1  

def rumor_center2(k):
        for i in Gr[k].children:
                Gr[i].rc=Gr[k].rc*(Gr[i].descedant_num/(n-float(Gr[i].descedant_num)))
                rumor_center2(i)
